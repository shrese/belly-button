...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    // 3. Create a variable that holds the samples array. WHY DATA.SAMPLES?
    var samplesArray = data.samples;
    console.log(samplesArray);

    // 4. Create a variable that filters the samples for the object with the desired sample number.
    var sampledData = samplesArray.filter(data => data.id == sample);

    console.log(sampledData);

    //  5. Create a variable that holds the first sample in the array.
    firstSample = sampledData[0];
    ...
    Plotly.newPlot("bubblePlot", bubbleData, bubbleLayout, { responsive: true });

    // ############# Challenge Deliverable 3 - GAUGE CHART ######################
    // 1. Create a variable that filters the metadata array for the object with the desired sample number.
    // var metaData = data.metadata
    var metaArray = data.metadata.filter(data => data.id == sample);
    console.log(metaArray)

    // 2. Create a variable that holds the washing frequency.
    washFrequency = metaArray[0].wfreq;
    ...
    Plotly.newPlot("guagePlot", gaugeData, gaugeLayout, { responsive: true });
  });
}